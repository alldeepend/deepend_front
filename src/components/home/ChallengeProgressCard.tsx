...
export const ChallengeProgressCard = () => {
    ...
    if (!data.challengeStarted) {
        return (
            <div className="lg:col-span-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center justify-center gap-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-24 h-24 bg-amber-50 rounded-bl-full -mr-6 -mt-6 opacity-60" />
                        ...
                        <div className="flex gap-3">
                            {[
                                ...
                                <div key={label} className="flex flex-col items-center">
                                    <span className="text-2xl font-bold text-slate-800 tabular-nums w-12 text-center bg-slate-50 rounded-xl py-1.5">
            ...
            <div className="lg:col-span-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col gap-4 relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-3">
                    ...
                    <div className="flex justify-between text-sm text-slate-500">
                        <span>
                            <span className="font-semibold text-emerald-600">{minutesThisWeek} min</span> registrados
                        </span>
                        {goalMinutes && (
                            <span>Meta: <span className="font-semibold text-emerald-600">{goalMinutes} min</span></span>
                        )}
                    </div>

                    {minimoViable && (
                        <div className="bg-slate-100 rounded-xl px-4 py-3 flex items-center gap-3">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-slate-500 flex-shrink-0">
                                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
                            </svg>
                            <div>
                                <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-wide">Tu mínimo viable</p>
                                <p className="text-sm font-bold text-red-500">{minimoViable}</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-2 pt-1 border-t border-slate-100">
                        ...
                        <span className="text-xs text-slate-400 ml-1">
                            <span className="font-semibold text-slate-600">{activeDays ?? 0}</span> de 7 días
                        </span>
                    </div>
                </div>
            </div>
            ...
            {isInfoOpen && (
                ...
                >
                    ...
                    >
                        ...
                        <h3 className="text-lg font-bold text-slate-800 mb-4">Tu progreso semanal</h3>
                        <div className="space-y-4 text-slate-600 text-sm leading-relaxed">
                            <p>Esta tarjeta muestra cómo vas en tu <strong className="text-slate-800">Reto Desde Aquí de 8 semanas</strong>. Todo se reinicia cada lunes.</p>
                            <ul className="space-y-3 mt-2">
                                <li className="flex items-start gap-3">
                                    <span className="w-2 h-2 rounded-full bg-emerald-500 mt-1.5 shrink-0"></span>
                                    <span><strong className="text-slate-800">Porcentaje</strong> — minutos registrados vs tu meta. Llegar al 100% es el reto. Superarlo, es el extra.</span>
                                </li>
                                <li className="flex items-start gap-3">
                                    <span className="w-2 h-2 rounded-full bg-emerald-500 mt-1.5 shrink-0"></span>
                                    <span><strong className="text-slate-800">Min registrados</strong> — suma de todos los minutos de actividad física que registraste esta semana.</span>
                                </li>
                                <li className="flex items-start gap-3">
                                    <span className="w-2 h-2 rounded-full bg-emerald-500 mt-1.5 shrink-0"></span>
                                    <span><strong className="text-slate-800">Meta</strong> — los minutos semanales que te comprometiste a mover. Puedes ajustarla cada lunes.</span>
                                </li>
                                <li className="flex items-start gap-3">
                                    <span className="w-2 h-2 rounded-full bg-red-400 mt-1.5 shrink-0"></span>
                                    <span><strong className="text-slate-800">Mínimo viable</strong> — cuando la semana se complica, este es tu piso mínimo. No negociable.</span>
                                </li>
                                <li className="flex items-start gap-3">
                                    <span className="w-2 h-2 rounded-full bg-slate-400 mt-1.5 shrink-0"></span>
                                    <span><strong className="text-slate-800">Días activos</strong> — cuántos días de esta semana (lunes a domingo) registraste al menos una actividad.</span>
                                </li>
                            </ul>
                        </div>
                    </div>
